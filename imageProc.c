...
struct image *contrast(struct image *im, double alpha)
{
 ...
 if (im->nlayers!=3){fprintf(stderr,"contrast(): Expedted 3-layer image!\n");return(NULL);}
...
struct image *computeWeightMap(struct image *im, double alphaC, double alphaS, double alphaE)
{
 ...
 // contrast, saturation, and well-exposedness terms from the original
 // exposure fusion formulation.
 ...
 // Multiply the three components to obtain the final weight map.
 pointwise_mul(t1,t2);
 deleteImage(t2);
 pointwise_mul(t1,t3);
 deleteImage(t3);
...
void deleteImage(struct image *im)
{
 ...
 for (int i=0; i<im->nlayers; i++)	// Release layer data
...
int writePPM(const char *name, struct image *im)
{
 ...
 for (ly=0; ly<3; ly++)
  for (j=0;j<image->sy;j++)
   for (i=0;i<image->sx;i++)
    *(tmp+((i+(j*image->sx))*3)+ly)=(unsigned char)(255.0*(*(image->layers[ly]+i+(j*image->sx))));
